  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <span className="font-body text-blanco/80 text-[13px]">{label}</span>
        <span className="font-body text-blanco/50 text-[12px] tabular-nums">{value}</span>
      </div>
      <div className="h-5 bg-gris-textura w-full">
...
function StatTile({
  ...
  return (
    <div className="bg-negro-suave border border-gris-textura p-6 lg:p-8">
      <Icon className="text-rojo mb-4" size={24} strokeWidth={1.5} />
      <p className="font-display text-blanco text-4xl md:text-5xl leading-none mb-2">{value}</p>
      <p className="font-body text-blanco/50 text-[11px] uppercase tracking-[0.25em]">{label}</p>
    </div>
  );
}
...
function AdminEncuestasPage() {
  ...
  if (!authed) {
    return (
      <div className="min-h-screen bg-negro grain flex items-center justify-center px-6">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            load(password);
        ...
        >
          ...
          <h1 className="font-display text-blanco text-2xl mb-6">PANEL DE ENCUESTAS</h1>
          <input
            type="password"
            value={password}
            ...
            autoFocus
            className="w-full bg-negro/60 border-2 border-blanco/25 focus:border-rojo outline-none px-4 py-3 font-body text-blanco text-base text-center placeholder:text-blanco/40 transition-colors duration-300 mb-4"
          />
  ...
  return (
    <div className="min-h-screen bg-negro grain">
      <header className="border-b border-gris-textura px-6 lg:px-12 py-8">
        <div className="max-w-[1200px] mx-auto flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <h1 className="font-display text-blanco text-2xl leading-none">{EVENT_LABEL}</h1>
            </div>
          </div>
          <button
            onClick={() => load(authed)}
            disabled={loading}
            className="flex items-center gap-2 font-body text-[12px] uppercase tracking-[0.2em] text-blanco/60 hover:text-rojo transition-colors"
          >
      ...
      {stats && (
        <main className="max-w-[1200px] mx-auto px-6 lg:px-12 py-12 space-y-16">
          ...
          {stats.total === 0 ? (
            <p className="font-body text-blanco/50 text-center py-20">
            ...
            <>
              ...
              <section>
                <h2 className="font-display text-blanco text-2xl mb-6">Calificación por categoría (1–5)</h2>
                <div className="bg-negro-suave border border-gris-textura p-6 lg:p-8 space-y-5">
              ...
              <div className="grid lg:grid-cols-2 gap-8">
                ...
                <section>
                  <h2 className="font-display text-blanco text-2xl mb-6">Calificación general</h2>
                  <div className="bg-negro-suave border border-gris-textura p-6 lg:p-8 space-y-5">
                ...
                {/* NPS breakdown */}
                <section>
                  <h2 className="font-display text-blanco text-2xl mb-6">Promotores / Pasivos / Detractores</h2>
                  <div className="bg-negro-suave border border-gris-textura p-6 lg:p-8 grid grid-cols-3 gap-4 text-center">
                    <div>
                      <p className="font-display text-rojo text-4xl mb-2">{stats.nps.promoters}</p>
                      <p className="font-body text-blanco/50 text-[10px] uppercase tracking-[0.2em]">Promotores (9–10)</p>
                    </div>
                    <div>
                      <p className="font-display text-blanco/70 text-4xl mb-2">{stats.nps.passives}</p>
                      <p className="font-body text-blanco/50 text-[10px] uppercase tracking-[0.2em]">Pasivos (7–8)</p>
                    </div>
                    <div>
                      <p className="font-display text-blanco/40 text-4xl mb-2">{stats.nps.detractors}</p>
                      <p className="font-body text-blanco/50 text-[10px] uppercase tracking-[0.2em]">Detractores (0–6)</p>
                    </div>
                  </div>
                </section>
                ...
                {/* Qué les gustó */}
                <section>
                  <h2 className="font-display text-blanco text-2xl mb-6">Qué les gustó más</h2>
                  <div className="bg-negro-suave border border-gris-textura p-6 lg:p-8 space-y-5">
                ...
                {/* Cómo se enteraron */}
                <section>
                  <h2 className="font-display text-blanco text-2xl mb-6">Cómo se enteraron del evento</h2>
                  <div className="bg-negro-suave border border-gris-textura p-6 lg:p-8 space-y-5">
                ...
                {/* Lugar */}
                <section>
                  <h2 className="font-display text-blanco text-2xl mb-6">
                ...
                {/* Compañía */}
                <section>
                  <h2 className="font-display text-blanco text-2xl mb-6">¿Con quién vinieron?</h2>
                  <div className="bg-negro-suave border border-gris-textura p-6 lg:p-8 space-y-5">
              ...
              {stats.recentComments.length > 0 && (
                <section>
                  <h2 className="font-display text-blanco text-2xl mb-6">Comentarios y sugerencias</h2>
                  <div className="grid md:grid-cols-2 gap-4">
                    {stats.recentComments.map((c, i) => (
                      <div key={i} className="bg-negro-suave border-l-2 border-rojo p-5">
                        <p className="font-body italic text-blanco/80 text-sm leading-relaxed">"{c.comment}"</p>
                      </div>
                    ))}
                  </div>
                </section>
              ...
              {stats.favoriteMoments.length > 0 && (
                <section>
                  <h2 className="font-display text-blanco text-2xl mb-6">Escenas y canciones más mencionadas</h2>
                  <div className="flex flex-wrap gap-2">
                    {stats.favoriteMoments.map((m, i) => (
                      <span key={i} className="font-body text-[12px] text-blanco/70 border border-gris-textura px-4 py-2">
                        {m}
                      </span>
                    ))}
                  </div>
